...
/*
// --- Alternative avec Axios (moins recommandée car moins spécifique à Google Places) ---
// Note: Cette implémentation serait similaire mais vous devriez gérer les URL et les réponses
// brutes des API Google Places manuellement, ce que la bibliothèque @googlemaps/google-maps-services-js fait pour vous.

async function getBusinessDetailsWithAxios(
    businessName: string,
    address: string,
    postalCode: string,
    apiKey: string
): Promise<BusinessDetails | null> {
    const query = encodeURIComponent(`${businessName}, ${address}, ${postalCode}`);
    const findPlaceUrl = `https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${query}&inputtype=textquery&fields=place_id,name,formatted_address&key=${apiKey}`;
    ...
    try {
        ...
        if (!candidates || candidates.length === 0) {
            console.log(`Aucun lieu trouvé avec Axios pour: "${query}"`);
            return null;
        }

        const placeId = candidates[0].place_id;
        const placeDetailsUrl = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=name,formatted_address,international_phone_number,website,opening_hours,rating,user_ratings_total,url&key=${apiKey}`;
        ...
        if (result) {
            ...
            console.log("Détails de l'entreprise récupérés avec Axios:");
            console.log(JSON.stringify(businessDetails, null, 2));
            return businessDetails;
        }
        return null;
        ...
        console.error("Erreur Axios lors de la récupération:", error.response?.data || error.message);
        return null;
    }
}

// Exemple d'utilisation de la version Axios
// runExampleAxios();
// async function runExampleAxios() {
//     const businessName = "Maison de la Radio";
//     const businessAddress = "Avenue du Président Kennedy";
//     const businessPostalCode = "75016";
//     const details = await getBusinessDetailsWithAxios(
//         businessName,
//         businessAddress,
//         businessPostalCode,
//         MY_API_KEY
//     );
//     if (details) {
//         console.log("\n--- Résultat Final (Axios) ---");
//         console.log(JSON.stringify(details, null, 2));
//     } else {
//         console.log("\n--- Impossible de récupérer les détails (Axios) ---");
//     }
// }
*/